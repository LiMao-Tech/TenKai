...
class UserChatModel: NSObject {
    ...
    var notifications = [NotificationFrame]()//用来存储系统和通知的数组，展示系统和通知
    var message = [Int :[SingleChatMessageFrame]]()//用来存储对应UserIndex的TenUser实例的聊天记录字典，key为UserIndex，value为UserIndex对应的TenUser的一个聊天信息的数组，用来展示聊天信息
